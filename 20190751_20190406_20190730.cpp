...
void Analyze(TreeNode *node, SymbolTable *symbol_table)
{
    ...
    if (node->node_kind == ID_NODE || node->node_kind == READ_NODE || node->node_kind == ASSIGN_NODE) // what can be inserted in ST
    {
    ...
    if (node->node_kind == OPER_NODE)
    {
        ...
        } // only less than or equal types can be oper else int
        else
        {
    ...
    {
        node->expr_data_type = INTEGER;
    } // int or id type is int

    if (node->node_kind == OPER_NODE)
    {
        if (node->child[0]->expr_data_type != INTEGER || node->child[1]->expr_data_type != INTEGER) // operands not int error
            printf(" error: Operands not integers in < or equal \n");
    }
    if (node->node_kind == IF_NODE && node->child[0]->expr_data_type != BOOLEAN)
    {
        printf("error: Operands have to be boolean in if \n");
    } // if have to has bool operands
    if (node->node_kind == REPEAT_NODE && node->child[1]->expr_data_type != BOOLEAN)
    {
        printf("error: Operands have to be boolean in repeat \n");
    }
    if (node->node_kind == WRITE_NODE && node->child[0]->expr_data_type != INTEGER)
    {
        printf("error: Operands not integers in write \n");
    } // write only with int
    if (node->node_kind == ASSIGN_NODE && node->child[0]->expr_data_type != INTEGER)
    {
        printf("error: Operands not integers in assign\n");
    } // assign only with int

    if (node->sibling)
        {Analyze(node->sibling, symbol_table);} // anlying sibling after we finish this node and it's children
}

////////////////////////////////////////////////////////////////////////////////////
// Code Generator //////////////////////////////////////////////////////////////////

int Power(int operand1, int operand2)//power calculation with recursion 
{
    if(operand1==0) return 0;
    if(operand2==0) return 1;
    if(operand2>=1) return operand1*Power(operand1, operand2-1);
    return 0;
}

int Evaluate(TreeNode *node, SymbolTable *symbol_table, int *variables)//performing operaitons
{
    ...
    } // if it's id we get the id return memloc 

    int operand1 = Evaluate(node->child[0], symbol_table, variables);
    int operand2 = Evaluate(node->child[1], symbol_table, variables);
...
void Run(TreeNode* node, SymbolTable* symbol_table, int* variables)
{
    if(node->node_kind==IF_NODE)
    {
        ...
        if (cond)
        {
            Run(node->child[1], symbol_table, variables);
        }
        else if (node->child[2])
        {
            Run(node->child[2], symbol_table, variables);
        }
    }
    if(node->node_kind==ASSIGN_NODE)
    {
        int value=Evaluate(node->child[0], symbol_table, variables);
        variables[symbol_table->Find(node->id)->memloc]=value;
    }
    ...
    if(node->node_kind==REPEAT_NODE)
    {
        do
        {
           Run(node->child[0], symbol_table, variables);
        }
        while(!Evaluate(node->child[1], symbol_table, variables));
    }
    if (node->sibling)
    {
        Run(node->sibling, symbol_table, variables);
    }
}

void Run(TreeNode* syntax_tree, SymbolTable* symbol_table)
{
    int* variables=new int[symbol_table->num_vars];
    for(int i=0;i<symbol_table->num_vars;i++) variables[i]=0;
    Run(syntax_tree, symbol_table, variables);
